...
export const Footer = () => {
  ...
  return (
    ...
    <footer className="flex flex-col items-center px-4 py-16 w-full bg-sky-50 md:px-20 md:pt-20 md:pb-6"> {/* Adjusted padding */}
    <div className="w-full max-w-[1498px]">
        <div className="flex flex-col md:flex-row justify-between items-start w-full gap-8 md:gap-0"> {/* Flex direction and gap */}

            <div className="w-full md:w-auto flex flex-col items-center md:items-start text-center md:text-left"> {/* Menu section width and alignment */}
                <div className="text-2xl md:text-3xl font-semibold">Menu</div> {/* Responsive font size */}
                <div className="mt-4 md:mt-7 text-base md:text-xl font-medium"> {/* Responsive font size and margin */}
                    {menuItems.map((item) => (
                        <div key={item.name} className="mb-2 md:mb-3"> {/* Responsive margin */}
                            <Link to={item.path} className="hover:text-blue-600">
                                {item.name}
                            </Link>
                        </div>
            ...
            <div className="w-full md:w-auto hidden md:block w-px bg-neutral-200 h-full"></div> {/* Vertical separator (hide on mobile) */}

            <div className="w-full md:w-auto flex flex-col items-center md:items-start text-center md:text-left"> {/* Contact info section width and alignment */}
                <div className="text-2xl md:text-3xl font-semibold text-neutral-900"> {/* Responsive font size */}
                    Contact Us
                </div>
                ...
                <div className="mt-6 md:mt-10 text-xl md:text-2xl font-semibold text-neutral-700"> {/* Responsive font size and margin */}
                    Social Links
                </div>
                <div className="flex justify-center md:justify-start gap-3 mt-4 md:mt-5"> {/* Responsive margin and justify content */}
                    {socialIcons.map((icon, index) => (
                        <div
                            className="flex flex-col justify-center items-center px-2 bg-yellow-400 rounded-full h-[39px] w-[39px]"
                            key={index}
            ...
            <div className="w-full md:w-auto flex flex-col items-center md:items-start text-center md:text-left"> {/* Newsletter section width and alignment */}
                <div className="text-2xl md:text-3xl font-semibold text-neutral-900"> {/* Responsive font size */}
                    Subscribe to Newsletter
                </div>
                <form onSubmit={handleSubscribe} className="flex flex-col md:flex-row gap-4 mt-4 md:mt-5 bg-white rounded-[30px] p-3 shadow-md w-full md:w-auto"> {/* Responsive flex direction, margin, and width */}
                    <input
                        type="email"
                        id="emailInput"
                        className="w-full md:w-auto text-base font-semibold text-zinc-500 px-3 py-2 border border-gray-300 rounded-md focus:outline-none" // Responsive width
                        placeholder="Your e-mail address"
                        aria-label="Your e-mail address"
                        value={email}
                    ...
                    <button
                        type="submit"
                        className="w-full md:w-auto px-6 py-2 text-white bg-black text-base md:text-xl font-medium rounded-[30px] hover:bg-gray-800" // Responsive width and font size
                    >
                        Subscribe
                    </button>
                </form>
        ...
        <div className="mt-8 md:mt-12 w-full h-px bg-neutral-200"></div> {/* Responsive margin */}

        <div className="text-center mt-3 md:mt-4 text-base font-medium text-black"> {/* Responsive margin */}
            Copyright © 2025 Tuler. All rights reserved.
        </div>
    </div>
</footer>
